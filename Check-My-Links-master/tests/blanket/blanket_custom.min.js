...
(exports => {
    ...
    let Token, TokenName, Syntax, PropertyKind, Messages, Regex, SyntaxTreeDelegate, source, strict, index, lineNumber, lineStart, length, delegate, lookahead, state, extra;
    ...
    function scanRegExp() {
        let str, ch, start, pattern, flags, value, classMarker = false, restore, terminated = false;
    ...
    function parsePostfixExpression() {
        let expr = parseLeftHandSideExpressionAllowCall(), token;
    ...
    function reduceBinary(stack) {
        const right = stack.pop(), operator = stack.pop().value, left = stack.pop();
    ...
    function parseBinaryExpression() {
        let expr, token, prec, previousAllowIn, stack;
    ...
    function parseConditionalExpression() {
        let expr, previousAllowIn, consequent, alternate;
    ...
    function parseExpression() {
        let expr = parseAssignmentExpression();
    ...
    function parseForVariableDeclaration() {
        const token = lex(), declarations = parseVariableDeclarationList();

        return delegate.createVariableDeclaration(declarations, token.value);
    }

    function parseForStatement() {
        let init, test, update, left, right, body, oldInIteration;
    ...
    function parseSwitchStatement() {
        let discriminant, cases, clause, oldInSwitch, defaultFound;
    ...
    function parseFunctionDeclaration() {
        let id, params = [], body, token, stricted, tmp, firstRestricted, message, previousStrict;
    ...
    function parseFunctionExpression() {
        let token, id = null, stricted, firstRestricted, message, tmp, params = [], body, previousStrict;
    ...
    function scanComment() {
        let comment, ch, loc, start, blockComment, lineComment;
    ...
    function collectToken() {
        let start, loc, token, range, value;
...
const parse = (typeof exports === 'undefined' ? esprima : require("./esprima")).parse;

(typeof exports === 'undefined' ? window : exports).falafel = (src, opts, fn, srcName) => {
...
(inBrowser ? window : exports).blanket = (() => {
    ...
    _blanket = {
        ...
        instrument(config, next) {
            const inFile = config.inputFile, inFileName = config.inputFileName;
        ...
        _trackingSetup(filename, sourceArray) {
            const branches = _blanket.options("branchTracking");
            ...
            if (branches){
                _blanket._branchingArraySetup.sort((a, b) => a.line > b.line).sort((a, b) => a.column > b.column).forEach(item => {
        ...
        _blockifyIf(node) {
            ...
            if (linesToAddBrackets.indexOf(node.type) > -1){
                const bracketsExistObject = node.consequent || node.body;
                const bracketsExistAlt = node.alternate;
...
(_blanket => {
    const oldOptions = _blanket.options;
_blanket.extend({
    ...
    showManualLoader() {
        ...
        let loader = "<div class='blanketDialogOverlay'>";
            loader += "&nbsp;</div>";
    ...
    manualFileLoader(files) {
        const toArray =Array.prototype.slice;
        files = toArray.call(files).filter(item => item.type !== "");
        const sessionLength = files.length-1;
        let sessionIndx=0;
        let sessionArray = {};
        ...
        const fileLoader = event => {
            ...
            const file = files[sessionIndx];
            const filename = file.webkitRelativePath && file.webkitRelativePath !== '' ? file.webkitRelativePath : file.name;
    ...
    report(coverage_data) {
        ...
        if (_blanket.options("reporter")){
            require([_blanket.options("reporter").replace(".js","")],r => {
                r(coverage_data);
            });
    ...
    _bindStartTestRunner(bindEvent, startEvent) {
        if (bindEvent){
    ...
    _loadSourceFiles(callback) {
        
        function copy(o){
          const _copy = Object.create( Object.getPrototypeOf(o) );
        ...
        const scripts = _blanket.utils.collectPageScripts();
        ...
        if (scripts.length === 0){
            ...
            const isOrdered = _blanket.options("orderedLoading");
            const initialGet=[];
            scripts.forEach((file, indx) => {
                //for whatever reason requirejs
                //prefers when we don't use the full path
                //so we just use a custom name
                const requireKey = "blanket_"+indx;
            ...
            const filt = initialGet;
            require(filt, () => {
                callback();
            });
        }
    },
    beforeStartTestRunner(opts) {
        opts = opts || {};
        ...
        if (opts.coverage) {
            ...
            () => {
                _blanket._loadSourceFiles(() => {
                    
                    const allLoaded = () => opts.condition ? opts.condition() : _blanket.requireFilesLoaded();
                    const check = () => {
                        if (allLoaded()) {
                            opts.callback();
                        } else {
...
(W => {
    function D(b){return M.call(b)==="[object Function]"}function E(b){return M.call(b)==="[object Array]"}function t(b,c){if(b){let d;for(d=0;d<b.length;d+=1)if(b[d]&&c(b[d],d,b))break}}function N(b,c){if(b){let d;for(d=b.length-1;d>-1;d-=1)if(b[d]&&c(b[d],d,b))break}}function A(b,c){for(const d in b)if(b.hasOwnProperty(d)&&c(b[d],d))break}function O(b,c,d,g){c&&A(c,(c, j) => {if(d||!F.call(b,j))g&&typeof c!=="string"?(b[j]||(b[j]={}),O(b[j],c,d,g)):b[j]=c});return b}function r(b,c){return function(){return c.apply(b,
    arguments)}}function X(b){if(!b)return b;let c=W;t(b.split("."),b => {c=c[b]});return c}function G(b,c,d,g){c=Error(c+"\nhttp://requirejs.org/docs/errors.html#"+b);c.requireType=b;c.requireModules=g;if(d)c.originalError=d;return c}function ba(){if(H&&H.readyState==="interactive")return H;N(document.getElementsByTagName("script"),b => {if(b.readyState==="interactive")return H=b});return H}
    let g;
    let s;
    let u;
    let y;
    ...
    let Z;
    const ca=/(\/\*([\s\S]*?)\*\/|([^:]|^)\/\/(.*)$)/mg;
    const da=/[^.]\s*require\s*\(\s*["']([^'"\s]+)["']\s*\)/g;
    const $=/\.js$/;
    const ea=/^\.\//;
    s=Object.prototype;
    var M=s.toString;
    var F=s.hasOwnProperty;
    const fa=Array.prototype.splice;
    const v=!!(typeof window!=="undefined"&&navigator&&document);
    const aa=!v&&typeof importScripts!=="undefined";
    const ga=v&&navigator.platform==="PLAYSTATION 3"?/^complete$/:/^(complete|loaded)$/;
    const R=typeof opera!=="undefined"&&opera.toString()==="[object Opera]";
    const w={};
    let n={};
    let P=[];
    let J=!1;
    if(typeof define==="undefined"){if(typeof requirejs!=="undefined"){if(D(requirejs))return;n=requirejs;requirejs=void 0}typeof require!=="undefined"&&
    !D(require)&&(n=require,require=void 0);g=requirejs=(b, c, d, p) => {let i, j="_";!E(b)&&typeof b!=="string"&&(i=b,E(c)?(b=c,c=d,d=p):b=[]);if(i&&i.context)j=i.context;(p=w[j])||(p=w[j]=g.s.newContext(j));i&&p.configure(i);return p.require(b,c,d)};g.config=b => g(b);g.nextTick=typeof setTimeout!=="undefined"?b => {setTimeout(b,4)}:b => {b()};require||(require=g);g.version="2.1.1";g.jsExtRegExp=/^\/|:|\?|\.js$/;g.isBrowser=v;s=g.s={contexts:w,newContext(b) {
        function c(a,
        f,x){let e, m, b, c, d, h, i, g=f&&f.split("/");e=g;const j=k.map, l=j&&j["*"];if(a&&a.charAt(0)===".")if(f){e=k.pkgs[f]?g=[f]:g.slice(0,g.length-1);f=a=e.concat(a.split("/"));for(e=0;f[e];e+=1)if(m=f[e],m===".")f.splice(e,1),e-=1;else if(m==="..")if(e===1&&(f[2]===".."||f[0]===".."))break;else e>0&&(f.splice(e-1,2),e-=2);e=k.pkgs[f=a[0]];a=a.join("/");e&&a===f+"/"+e.main&&(a=f)}else a.indexOf("./")===0&&(a=a.substring(2));if(x&&(g||l)&&j){f=a.split("/");for(e=f.length;e>0;e-=1){b=f.slice(0,e).join("/");if(g)for(m=
        g.length;m>0;m-=1)if(x=j[g.slice(0,m).join("/")])if(x=x[b]){c=x;d=e;break}if(c)break;!h&&l&&l[b]&&(h=l[b],i=e)}!c&&h&&(c=h,d=i);c&&(f.splice(0,d,c),a=f.join("/"))}return a}function d(a){v&&t(document.getElementsByTagName("script"),f => {if(f.getAttribute("data-requiremodule")===a&&f.getAttribute("data-requirecontext")===h.contextName)return f.parentNode.removeChild(f),!0})}function p(a){const f=k.paths[a];if(f&&E(f)&&f.length>1)return d(a),f.shift(),h.require.undef(a),h.require([a]),!0}function i(a){
            let f;
            const b=a?a.indexOf("!"):-1;
            b>-1&&(f=a.substring(0,b),a=a.substring(b+1,a.length));return[f,a]
        }function j(a,f,b,e){
            let m;
            let K;
            let d=null;
            const g=f?f.name:null;
            const j=a;
            let l=!0;
            let k="";
            a||(l=!1,a="_@r"+(M+=1));a=i(a);d=a[0];a=a[1];d&&(d=c(d,g,e),K=o[d]);a&&(d?k=K&&K.normalize?K.normalize(a,a => c(a,g,e)):c(a,g,e):(k=c(a,g,e),a=i(k),d=a[0],k=a[1],b=!0,m=h.nameToUrl(k)));b=d&&!K&&!b?"_unnormalized"+(N+=1):"";return{prefix:d,name:k,parentMap:f,unnormalized:!!b,url:m,originalName:j,isDefine:l,id:(d?d+"!"+k:k)+b}
        }function n(a){
            const f=
            a.id;

            let b=l[f];
            b||(b=l[f]=new h.Module(a));return b
        }function q(a,f,b){const e=a.id, m=l[e];if(F.call(o,e)&&(!m||m.defineEmitComplete))f==="defined"&&b(o[e]);else n(a).on(f,b)}function z(a,f){
            const b=a.requireModules;
            let e=!1;
            if(f)f(a);else if(t(b,f => {if(f=l[f])f.error=a,f.events.error&&(e=!0,f.emit("error",a))}),!e)g.onError(a)
        }function s(){P.length&&(fa.apply(C,[C.length-1,0].concat(P)),P=[])}function u(a,f,b){const e=a.map.id;a.error?a.emit("error",a.error):(f[e]=!0,t(a.depMaps,(e, c) => {const d=e.id, g=l[d];g&&!a.depMatched[c]&&!b[d]&&(f[d]?(a.defineDep(c,o[d]),a.check()):u(g,f,b))}),b[e]=!0)}function w(){
            let a;
            let f;
            let b;
            let e;
            const m=(b=k.waitSeconds*1E3)&&h.startTime+b<(new Date).getTime();
            const c=[];
            const g=[];
            let i=!1;
            let j=!0;
            if(!S){S=!0;A(l,b => {a=b.map;f=a.id;if(b.enabled&&(a.isDefine||g.push(b),!b.error))if(!b.inited&&m)p(f)?i=e=!0:(c.push(f),d(f));else if(!b.inited&&b.fetched&&a.isDefine&&(i=!0,!a.prefix))return j=!1});if(m&&c.length)return b=G("timeout","Load timeout for modules: "+c,null,c),b.contextName=h.contextName,
            z(b);j&&t(g,a => {u(a,{},{})});if((!m||e)&&i)if((v||aa)&&!T)T=setTimeout(() => {T=0;w()},50);S=!1}
        }function y(a){n(j(a[0],null,!0)).init(a[1],a[2])}function B(a){
            const a=a.currentTarget||a.srcElement;
            let b=h.onScriptLoad;
            a.detachEvent&&!R?a.detachEvent("onreadystatechange",b):a.removeEventListener("load",b,!1);b=h.onScriptError;a.detachEvent&&!R||a.removeEventListener("error",b,!1);return{node:a,id:a&&a.getAttribute("data-requiremodule")}
        }function I(){let a;for(s();C.length;)if(a=C.shift(),a[0]===
        ...
        let L;
        var T;
        var k={waitSeconds:7,baseUrl:"./",paths:{},pkgs:{},shim:{},map:{},config:{}};
        var l={};
        const V={};
        var C=[];
        var o={};
        ...
        var M=1;
        var N=1;
        L={require(a) {return a.require?a.require:a.require=h.makeRequire(a.map)},exports(a) {a.usingExports=!0;if(a.map.isDefine)return a.exports?a.exports:a.exports=o[a.map.id]={}},module(a) {return a.module?a.module:a.module={id:a.map.id,uri:a.map.url,config() {return k.config&&
        k.config[a.map.id]||{}},exports:o[a.map.id]};}};U=function(a){this.events=V[a.id]||{};this.map=a;this.shim=k.shim[a.id];this.depExports=[];this.depMaps=[];this.depMatched=[];this.pluginMaps={};this.depCount=0};U.prototype={init(a, b, c, e) {e=e||{};if(!this.inited){this.factory=b;if(c)this.on("error",c);else this.events.error&&(c=r(this,function(a){this.emit("error",a)}));this.depMaps=a&&a.slice(0);this.errback=c;this.inited=!0;this.ignore=e.ignore;e.enabled||this.enabled?this.enable():this.check()}},
        defineDep(a, b) {this.depMatched[a]||(this.depMatched[a]=!0,this.depCount-=1,this.depExports[a]=b)},fetch() {if(!this.fetched){this.fetched=!0;h.startTime=(new Date).getTime();const a=this.map;if(this.shim)h.makeRequire(this.map,{enableBuildCallback:!0})(this.shim.deps||[],r(this,function(){return a.prefix?this.callPlugin():this.load()}));else return a.prefix?this.callPlugin():this.load()}},load() {const a=this.map.url;Q[a]||(Q[a]=!0,h.load(this.map.id,a))},check() {if(this.enabled&&
        !this.enabling){
            let a;
            let b;
            const c=this.map.id;
            b=this.depExports;
            let e=this.exports;
            const m=this.factory;
            if(this.inited)if(this.error)this.emit("error",this.error);else{if(!this.defining){this.defining=!0;if(this.depCount<1&&!this.defined){if(D(m)){if(this.events.error)try{e=h.execCb(c,m,b,e)}catch(d){a=d}else e=h.execCb(c,m,b,e);if(this.map.isDefine)if((b=this.module)&&b.exports!==void 0&&b.exports!==this.exports)e=b.exports;else if(e===void 0&&this.usingExports)e=this.exports;if(a)return a.requireMap=this.map,
        ...
        }},callPlugin() {const a=this.map, b=a.id, d=j(a.prefix);this.depMaps.push(d);q(d,"defined",r(this,function(e){let m, d;d=this.map.name;const x=this.map.parentMap?this.map.parentMap.name:null, i=h.makeRequire(a.parentMap,{enableBuildCallback:!0,skipMap:!0});if(this.map.unnormalized){if(e.normalize&&(d=e.normalize(d,a => c(a,x,!0))||""),e=j(a.prefix+"!"+d,this.map.parentMap),q(e,"defined",r(this,function(a){this.init([],() => a,null,{enabled:!0,ignore:!0})})),d=l[e.id]){this.depMaps.push(e);if(this.events.error)d.on("error",r(this,function(a){this.emit("error",a)}));d.enable()}}else m=r(this,
        function(a){this.init([],() => a,null,{enabled:!0})}),m.error=r(this,function(a){this.inited=!0;this.error=a;a.requireModules=[b];A(l,a => {a.map.id.indexOf(b+"_unnormalized")===0&&delete l[a.map.id]});z(a)}),m.fromText=r(this,function(b,e){const f=a.name, c=j(f), d=J;e&&(b=e);d&&(J=!1);n(c);try{g.exec(b)}catch(x){throw Error("fromText eval for "+f+" failed: "+x);}d&&(J=!0);this.depMaps.push(c);h.completeLoad(f);i([f],m)}),e.load(a.name,i,m,k)}));h.enable(d,this);this.pluginMaps[d.id]=
        d},enable() {this.enabling=this.enabled=!0;t(this.depMaps,r(this,function(a,b){let c, e;if(typeof a==="string"){a=j(a,this.map.isDefine?this.map:this.map.parentMap,!1,!this.skipMap);this.depMaps[b]=a;if(c=L[a.id]){this.depExports[b]=c(this);return}this.depCount+=1;q(a,"defined",r(this,function(a){this.defineDep(b,a);this.check()}));this.errback&&q(a,"error",this.errback)}c=a.id;e=l[c];!L[c]&&e&&!e.enabled&&h.enable(a,this)}));A(this.pluginMaps,r(this,function(a){const b=l[a.id];b&&!b.enabled&&
        h.enable(a,this)}));this.enabling=!1;this.check()},on(a, b) {let c=this.events[a];c||(c=this.events[a]=[]);c.push(b)},emit(a, b) {t(this.events[a],a => {a(b)});a==="error"&&delete this.events[a]}};h={config:k,contextName:b,registry:l,defined:o,urlFetched:Q,defQueue:C,Module:U,makeModuleMap:j,nextTick:g.nextTick,configure(a) {a.baseUrl&&a.baseUrl.charAt(a.baseUrl.length-1)!=="/"&&(a.baseUrl+="/");const b=k.pkgs, c=k.shim, e={paths:!0,config:!0,map:!0};A(a,(a, b) => {e[b]?
        b==="map"?O(k[b],a,!0,!0):O(k[b],a,!0):k[b]=a});if(a.shim)A(a.shim,(a, b) => {E(a)&&(a={deps:a});if(a.exports&&!a.exportsFn)a.exportsFn=h.makeShimExports(a);c[b]=a}),k.shim=c;if(a.packages)t(a.packages,a => {a=typeof a==="string"?{name:a}:a;b[a.name]={name:a.name,location:a.location||a.name,main:(a.main||"main").replace(ea,"").replace($,"")}}),k.pkgs=b;A(l,(a, b) => {if(!a.inited&&!a.map.unnormalized)a.map=j(b)});if(a.deps||a.callback)h.require(a.deps||[],a.callback)},makeShimExports(a) {return function(){let b;
        a.init&&(b=a.init.apply(W,arguments));return b||X(a.exports)};},makeRequire(a, f) {function d(e,c,i){let k, p;if(f.enableBuildCallback&&c&&D(c))c.__requireJsBuild=!0;if(typeof e==="string"){if(D(c))return z(G("requireargs","Invalid require call"),i);if(a&&L[e])return L[e](l[a.id]);if(g.get)return g.get(h,e,a);k=j(e,a,!1,!0);k=k.id;return!F.call(o,k)?z(G("notloaded",'Module name "'+k+'" has not been loaded yet for context: '+b+(a?"":". Use require([])"))):o[k]}I();h.nextTick(() => {I();p=
        n(j(null,a));p.skipMap=f.skipMap;p.init(e,c,i,{enabled:!0});w()});return d}f=f||{};O(d,{isBrowser:v,toUrl(b) {
            const d=b.lastIndexOf(".");
            let f=null;
            d!==-1&&(f=b.substring(d,b.length),b=b.substring(0,d));return h.nameToUrl(c(b,a&&a.id,!0),f)
        },defined(b) {b=j(b,a,!1,!0).id;return F.call(o,b)},specified(b) {b=j(b,a,!1,!0).id;return F.call(o,b)||F.call(l,b)}});if(!a)d.undef=b => {s();const c=j(b,a,!0), d=l[b];delete o[b];delete Q[c.url];delete V[b];if(d){if(d.events.defined)V[b]=
        d.events;delete l[b]}};return d},enable(a) {l[a.id]&&n(a).enable()},completeLoad(a) {
            let b;
            let c;
            const d=k.shim[a]||{};
            const g=d.exports;
            for(s();C.length;){c=C.shift();if(c[0]===null){c[0]=a;if(b)break;b=!0}else c[0]===a&&(b=!0);y(c)}c=l[a];if(!b&&!o[a]&&c&&!c.inited)if(k.enforceDefine&&(!g||!X(g)))if(p(a))return;else return z(G("nodefine","No define call for "+a,null,[a]));else y([a,d.deps||[],d.exportsFn]);w()
        },nameToUrl(a, b) {let c, d, i, h, j, l;if(g.jsExtRegExp.test(a))h=a+(b||"");else{c=
        k.paths;d=k.pkgs;h=a.split("/");for(j=h.length;j>0;j-=1)if(l=h.slice(0,j).join("/"),i=d[l],l=c[l]){E(l)&&(l=l[0]);h.splice(0,j,l);break}else if(i){c=a===i.name?i.location+"/"+i.main:i.location;h.splice(0,j,c);break}h=h.join("/");h+=b||(/\?/.test(h)?"":".js");h=(h.charAt(0)==="/"||h.match(/^[\w\+\.\-]+:/)?"":k.baseUrl)+h}return k.urlArgs?h+((h.indexOf("?")===-1?"?":"&")+k.urlArgs):h},load(a, b) {g.load(h,a,b)},execCb(a, b, c, d) {return b.apply(d,c)},onScriptLoad(a) {if(a.type===
        "load"||ga.test((a.currentTarget||a.srcElement).readyState))H=null,a=B(a),h.completeLoad(a.id)},onScriptError(a) {const b=B(a);if(!p(b.id))return z(G("scripterror","Script error",a,[b.id]))}};h.require=h.makeRequire();return h
    }};g({});t(["toUrl","undef","defined","specified"],b => {g[b]=function(){const c=w._;return c.require[b].apply(c,arguments)}});if(v&&(u=s.head=document.getElementsByTagName("head")[0],y=document.getElementsByTagName("base")[0]))u=s.head=y.parentNode;g.onError=b => {throw b;
    };g.load=(b, c, d) => {
        const g=b&&b.config||{};
        let i;
    ...
    };v&&N(document.getElementsByTagName("script"),b => {if(!u)u=b.parentNode;if(q=b.getAttribute("data-main")){if(!n.baseUrl)B=q.split("/"),Y=B.pop(),Z=B.length?B.join("/")+"/":"./",n.baseUrl=Z,q=Y;q=q.replace($,"");n.deps=n.deps?n.deps.concat(q):[q];return!0}});define=(b, c, d) => {let g, i;typeof b!=="string"&&(d=c,c=b,b=
    null);E(c)||(d=c,c=[]);!c.length&&D(d)&&d.length&&(d.toString().replace(ca,"").replace(da,(b, d) => {c.push(d)}),c=(d.length===1?["require"]:["require","exports","module"]).concat(c));if(J&&(g=I||ba()))b||(b=g.getAttribute("data-requiremodule")),i=w[g.getAttribute("data-requirecontext")];(i?i.defQueue:P).push([b,c,d])};define.amd={jQuery:!0};g.exec=b => eval(b);g(n)}
})(this);

    
}
blanket.defaultReporter = coverage => {
    const cssSytle = "#blanket-main {margin:2px;background:#EEE;color:#333;clear:both;font-family:'Helvetica Neue Light', 'HelveticaNeue-Light', 'Helvetica Neue', Calibri, Helvetica, Arial, sans-serif; font-size:17px;} #blanket-main a {color:#333;text-decoration:none;}  #blanket-main a:hover {text-decoration:underline;} .blanket {margin:0;padding:5px;clear:both;border-bottom: 1px solid #FFFFFF;} .bl-error {color:red;}.bl-success {color:#5E7D00;} .bl-file{width:auto;} .bl-cl{float:left;} .blanket div.rs {margin-left:50px; width:150px; float:right} .bl-nb {padding-right:10px;} #blanket-main a.bl-logo {color: #EB1764;cursor: pointer;font-weight: bold;text-decoration: none} .bl-source{ overflow-x:scroll; background-color: #FFFFFF; border: 1px solid #CBCBCB; color: #363636; margin: 25px 20px; width: 80%;} .bl-source div{white-space:nowrap;} .bl-source span{background-color: #EAEAEA;color: #949494;display: inline-block;padding: 0 10px;text-align: center; } .bl-source .miss{background-color:#e6c3c7} .bl-source span.branchWarning{color:#000;background-color:yellow;} .bl-source span.branchOkay{color:#000;background-color:transparent;}";
    const successRate = 60;
    const head = document.head;
    let fileNumber = 0;
    const body = document.body;
    let headerContent;
    let bodyContent = "<div id='blanket-main'><div class='blanket bl-title'><div class='bl-cl bl-file'><a href='http://migrii.github.com/blanket/' target='_blank' class='bl-logo'>Blanket.js</a> results</div><div class='bl-cl rs'>Coverage (%)</div><div class='bl-cl rs'>Covered/Total Smts.</div><div style='clear:both;'></div></div>";
    const fileTemplate = "<div class='blanket {{statusclass}}'><div class='bl-cl bl-file'><span class='bl-nb'>{{fileNumber}}.</span><a>{{file}}</a></div><div class='bl-cl rs'>{{percentage}} %</div><div class='bl-cl rs'>{{numberCovered}}/{{totalSmts}}</div><div id='file-{{fileNumber}}' class='bl-source'>{{source}}</div><div style='clear:both;'></div></div>";
    ...
    // Chrome Extensions don't allow inline scripts
    ...
    const percentage = (number, total) => Math.round(((number/total) * 100)*100)/100;

    const appendTag = (type, el, str) => {
    ...
    function branchReport(colsIndex,src,cols,offset,lineNum){
      ...
      const thisline = cols[colsIndex];
      //consequent
      
      const cons = thisline.consequent;
      if (cons.start.line > lineNum){
        ...
        let style = "<span class='" + (isBranchFollowed(thisline,true) ? 'branchOkay' : 'branchWarning') + "'>";
        ...
        {
          const res = branchReport(colsIndex+1,src.slice(cons.start.column-offset,cons.end.column-offset),cols,cons.start.column-offset,lineNum);
          newsrc += res.src;
          cols = res.cols;
        ...
        if (alt.start.line > lineNum){
          ...
          {
            const res2 = branchReport(colsIndex+1,src.slice(alt.start.column-offset,alt.end.column-offset),cols,alt.start.column-offset,lineNum);
            newsrc += res2.src;
            cols = res2.cols;
            cols[colsIndex+1] = cols[colsIndex+2];
            cols.length--;
    ...
    for(const file in files)
    {
        ...
        const statsForFile = files[file];
        let totalSmts = 0;
        let numberOfFilesCovered = 0;
        const code = [];
        let i;
        ...
        for(i = 0; i < statsForFile.source.length; i +=1){
                ...
                typeof statsForFile.branchData !== 'undefined')
            {
                ...
                {
                  const cols = statsForFile.branchData[i+1].filter(isUndefined);
                  const colsIndex=0;
        ...
        const result = percentage(numberOfFilesCovered, totalSmts);

        let output = fileTemplate.replace("{{file}}", file)
...
(() => {
    ...
    const toArray =Array.prototype.slice;
    const scripts = toArray.call(document.scripts);
...
_blanket.extend({
    ...
    matchPatternAttribute(filename, pattern) {
        if (typeof pattern === 'string'){
            if (pattern.indexOf("[") === 0){
                //treat as array
                const pattenArr = pattern.slice(1,pattern.length-1).split(",");
                return pattenArr.some(elem => filename.indexOf(_blanket.utils.normalizeBackslashes(elem.slice(1,-1))) > -1);
            ...
            return pattern.some(elem => _blanket.utils.matchPatternAttribute(filename,elem));
    ...
    collectPageScripts() {
        const toArray = Array.prototype.slice;
        const scripts = toArray.call(document.scripts);
        let selectedScripts=[], scriptNames=[];
        const filter = _blanket.options("filter");
        if(filter){
            //global filter in place, data-cover-only
            const antimatch = _blanket.options("antifilter");
            selectedScripts = toArray.call(document.scripts)
                            .filter(s => toArray.call(s.attributes).filter(sn => sn.nodeName === "src" && _blanket.utils.matchPatternAttribute(sn.nodeValue,filter) &&
                (typeof antimatch === "undefined" || !_blanket.utils.matchPatternAttribute(sn.nodeValue,antimatch))).length === 1);
        }else{
            selectedScripts = toArray.call(document.querySelectorAll("script[data-cover]"));
        }
        scriptNames = selectedScripts.map(s => _blanket.utils.qualifyURL(
            toArray.call(s.attributes).filter(
                sn => sn.nodeName === "src")[0].nodeValue).replace(".js",""));
        if (!filter){
...
requirejs.load = (context, moduleName, url) => {
    ...
    requirejs.cget(url, content => {
        _blanket.requiringFile(url,true);
        const match = _blanket.options("filter");
...
requirejs.cget = (url, callback, errback, onXhr) => {
    let foundInSession = false;
    if (_blanket.blanketSession){
        const files = Object.keys(_blanket.blanketSession);
...
if (typeof QUnit !== 'undefined'){
    ...
    const allLoaded = () => window.QUnit.config.queue.length > 0 && blanket.noConflict().requireFilesLoaded();
